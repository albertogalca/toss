...
contextBridge.exposeInMainWorld("toss", {
  ...
  addFile: (filePath, password) =>
    ipcRenderer.invoke("add-file", filePath, password),
  ...
  getFilePassword: (shareId) =>
    ipcRenderer.invoke("get-file-password", shareId),

  setFilePassword: (shareId, password) =>
    ipcRenderer.invoke("set-file-password", shareId, password),
  ...
  sendSignal: (shareId, sessionId, data) =>
    ipcRenderer.invoke("send-signal", shareId, sessionId, data),

  readFileChunk: (shareId, offset, length) =>
    ipcRenderer.invoke("read-file-chunk", shareId, offset, length),

  verifyPassword: (shareId, pw) =>
    ipcRenderer.invoke("verify-password", shareId, pw),
